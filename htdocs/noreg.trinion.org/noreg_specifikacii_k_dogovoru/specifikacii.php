...
// Check for specification ID parameter
...
// Check if specification was found
if (!$spec_info) {
    $page_title = 'Ошибка';
    include '../header.php';
...
<div class="container-fluid">
    <div class="row mb-3 d-print-none" style="margin-top: 30px;">
        <div class="col-auto ms-auto">
            ...
            <button type="button" class="btn btn-primary" onclick="window.location.href='form.php?id=<?= htmlspecialchars($_GET['id']) ?>';">
    ...
    <div class="card card-lg">
        <div class="card-body">
            ...
            <div style="font-family: 'Times New Roman', serif; line-height: 1.6; white-space: pre-wrap; color: black; text-align: justify;">
                ...
                $preamble .=  ", КПП " . htmlspecialchars($spec_info['kon_kpp']) . ", адрес: " . htmlspecialchars($spec_info['kon_address']) . ", именуемое в дальнейшем «Покупатель», в лице " . htmlspecialchars($spec_info['kon_representative']) . " действующего на основании Устава, с другой стороны, вместе именуемые в дальнейшем «Стороны», действуя в соответствии со статьями 1 и 2 договора поставки № " . htmlspecialchars($spec_info['nomer_specifikacii']) . ", заключённого между Сторонами " . htmlspecialchars($spec_info['data_dogovora']) . " (далее - Договор), согласовали настоящую спецификацию в том, что Поставщик обязуется передать, а Покупатель принять и оплатить Товар на следующих условиях:";
            ...
            <table class="table table-bordered table-responsive" style="margin-top: 30px;">
                ...
                foreach ($items as $month => $group) {
                    ...
                    // Calculate group VAT sum and effective rate
                    $group_vat_sum = 0;
                    foreach ($group as $item) {
                ...
                // Calculate total VAT amount and weighted average rate
                $sum_per = 0;
                $grand_quantity = 0;
                foreach ($items as $month => $group) {
                    foreach ($group as $item) {
            ...
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($spec_info['usloviya_otgruzki'] ?? '') ?></td>
                    </tr>
                    <tr>
                        <td style="font-weight: bold;">Условия оплаты:</td>
                        <td><?= htmlspecialchars($spec_info['usloviya_oplaty'] ?? '') ?></td>
                    </tr>
                    <tr>
                        <td style="font-weight: bold;">Иные условия:</td>
                        <td><?= htmlspecialchars($spec_info['inye_usloviya'] ?? '') ?></td>
                    </tr>
                </tbody>
            </table>
