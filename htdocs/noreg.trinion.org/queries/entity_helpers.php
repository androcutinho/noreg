...
/**
 * Helper functions for managing entities (warehouse, vendor, organization, product, unit, series)
 * These functions abstract the check-or-create pattern to reduce code duplication
 */

/**
 * Get existing entity ID or create new entity if it doesn't exist
 * 
 * @param $mysqli MySQLi connection
 * @param $table_name Table constant (e.g., TABLE_WAREHOUSES)
 * @param $id_column Column constant for ID (e.g., COL_WAREHOUSE_ID)
 * @param $name_column Column constant for name (e.g., COL_WAREHOUSE_NAME)
 * @param $entity_name Entity name to check/insert
 * @return int Entity ID (existing or newly created)
 * @throws Exception
 */
 ...
 * 
 * @param $mysqli MySQLi connection
 * @param $seria_id Existing series ID (if any)
 * @param $seria_name Series name/number
 * @param $product_id Product ID for the series
 * @param $prod_date Production date (optional)
 * @param $exp_date Expiration date (optional)
 * @param $insert_dates Whether to include dates in INSERT (true for create, false for edit)
