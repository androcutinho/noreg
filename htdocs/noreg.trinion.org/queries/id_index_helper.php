...
function getNextIdIndex($mysqli) {
    $query = "SELECT COALESCE(MAX(id_index), 0) + 1 as next_index FROM stroki_dokumentov";
...
function getTovaryZakazasostatkom($mysqli, $zakaz, $original_items, $document_type = 'shipment') {
    ...
    if (!empty($doc_types[0])) {
        if ($doc_types[0] === 'shipment') {
            ...
            $query_inv = "SELECT id_index FROM scheta_na_oplatu_pokupatelyam WHERE id IN ($placeholders)";
