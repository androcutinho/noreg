...
$order = fetchZakazHeader($mysqli, $zakaz_id);

if (!$order) {
    die("Заказ не найден.");
}

$line_items = fetchZakazLineItems($mysqli, $order['id_index']);
...
// Get unique NDS rates used in this order
            ...
            <div class="card-body">
                ...
                <div style="margin-bottom: 30px; border-bottom: 2px solid #000; padding-bottom: 15px;">
                    <h2 style="margin: 0; font-weight: bold;">
                        Заказ поставщику № <?= htmlspecialchars($order['nomer']) ?> от <?= htmlspecialchars($formatted_date) ?>
                    </h2>
                </div>
                ...
                <div style="margin-bottom: 30px; border-bottom: 2px solid #000; padding-bottom: 15px;">
                    <p>
                        Всего наименований: <?= count($line_items) ?>, на сумму <?= number_format($total_sum, 2, ',', ' ') ?> RUB
                    </p>
                </div>
                ...
                <div style="margin-bottom: 40px; padding: 15px;">
                    <p>
                        <strong>Ответственный:</strong> <?= htmlspecialchars($order['responsible_name'] ?? '') ?>
                    </p>
                </div>

                <?php 
                $related_documents = [];
                if (!empty($order['id_scheta_na_oplatu_postavshchikam'])) {
                    $schet_ids_json = $order['id_scheta_na_oplatu_postavshchikam'];
