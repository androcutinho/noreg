...
// mesyats mapping for simple format (lowercase)
$months_ru_simple = [
    'January' => 'января',
    'February' => 'февраля',
    'March' => 'марта',
...
// mesyats mapping for table display (capitalized)
$mesyats_ru_zaglavnyj = [
    'January' => 'Январь',
    'February' => 'Февраль',
    'March' => 'Март',
    'April' => 'Апрель',
...
function formatDateRussian($date_str, $months_array) {
    ...
    $day = date('j', $date);
    $mesyats = $months_array[date('F', $date)];
    $year = date('Y', $date);
    return $day . ' ' . $mesyats . ' ' . $year . ' года';
}
...
function formatDateFormalRussian($date_str) {
    ...
    // mesyats names in Russian (genitive case)
    $months_ru = [
    ...
    return $days_ru[$day] . ' ' . $months_ru[$mesyats] . ' ' . trim($year_str);
}
?>
