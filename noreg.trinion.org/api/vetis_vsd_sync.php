...
function syncDocumentsToDatabase($documents, $mysqli)
{
    ...
    foreach ($documents as $index => $vet_doc) {
        try {
            ...
            $enterprise_guid = $vet_doc['enterprise_guid'] ?? '';
            ...
            if ($result->num_rows > 0) {
                ...
                if (strcmp($last_update_date, $stored_last_update) > 0) {
                    $update_sql = "UPDATE vetis_vsd SET issueDate = ?, vetDType = ?, vetDStatus = ?, lastUpdateDate = ?, issueNumber = ?, dateOfProduction = ?, expiryDate = ?, enterprise = ?, consignee = ?, naimenovanie_tovara = ?, id_tovary_i_uslugi = ?, enterprise_guid = ? WHERE uuid = ?";
                    ...
                    $update_stmt->bind_param('ssssssssssiss', $issue_date, $vet_dtype, $vet_dstatus, $last_update_date, $issue_number, $prod_date, $exp_date, $shipper_name, $receiver_name, $product_name, $id_tovary_i_uslugi, $enterprise_guid, $doc_uuid);
                ...
                $insert_sql = "INSERT INTO vetis_vsd (uuid, issueDate, vetDType, vetDStatus, lastUpdateDate, issueNumber, dateOfProduction, expiryDate, enterprise, consignee, naimenovanie_tovara, id_tovary_i_uslugi, enterprise_guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                $insert_stmt->bind_param('sssssssssssis', $doc_uuid, $issue_date, $vet_dtype, $vet_dstatus, $last_update_date, $issue_number, $prod_date, $exp_date, $shipper_name, $receiver_name, $product_name, $id_tovary_i_uslugi, $enterprise_guid);
