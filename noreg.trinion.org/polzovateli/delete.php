...
if (!$logged_in_user || !$logged_in_user['user_role']) {
    header('Location: spisok.php?error=Доступ запрещен. Только администраторы могут удалять пользователей.');
    exit;
}
...
if ($user_id == $_SESSION['user_id']) {
    header('Location: spisok.php?error=Невозможно удалить свой аккаунт');
    exit;
}
