...
if ($zakaz_id && !$is_edit) {
    ...
    if ($order_source === 'supplier') {
        $zakaz = fetchZakazHeader($mysqli, $zakaz_id); // from zakaz_query.php (supplier orders)
    } else {
        $zakaz = fetchOrderHeader($mysqli, $zakaz_id); // from zakaz_pokupatelya_query.php (customer orders) 
    }
