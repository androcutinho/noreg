...
// Get total count of zakazy
$summa_zakazy = getSummaSchetov($mysqli, $type);
$obshchee_stranits = ceil($summa_zakazy / $items_na_stranitse);
...
// Get zakazy for current page
$schetov = getAllschetov($mysqli, $items_na_stranitse, $offset, $type);
            ...
            <div class="card-header">
              <div class="row w-full">
                <div class="col">
                  ...
                  <p class="text-secondary m-0">Всего счетов: <?= $summa_zakazy ?> штук.</p>
                </div>
              ...
              <table class="table table-vcenter card-table">
                ...
                <tbody>
                  <?php if (!empty($schetov)): ?>
                    <?php foreach ($schetov as $zakaz): ?>
                      <tr>
                        <td><a href="prosmotr.php?id=<?= htmlspecialchars($zakaz['id']) ?>" class="text-primary"><?= htmlspecialchars($zakaz['nomer']) ?></a></td>
                        <td class="text-secondary"><?= htmlspecialchars($zakaz['data_dokumenta']) ?></td>
                        <td class="text-secondary"><?= htmlspecialchars(($type === 'postavschik' ? $zakaz['naimenovanie_organizacii'] : $zakaz['naimenovanie_postavschika']) ?? 'N/A') ?></td>
                        <td class="text-secondary"><?= htmlspecialchars(($type === 'postavschik' ? $zakaz['naimenovanie_postavschika'] : $zakaz['naimenovanie_organizacii']) ?? 'N/A') ?></td>
