...
$type = isset($_GET['type']) && $_GET['type'] === 'iskhodyashchij' ? 'iskhodyashchij' : 'vhodyashchij';
$type_label = ($type === 'iskhodyashchij') ? 'исходящие' : 'входящие';
$page_title = 'Платежи (' . $type_label . ')';

$mysqli = require '../config/database.php';
require '../queries/platezhi_queries.php';

$tekushchaya_stranitsa = isset($_GET['page']) ? intval($_GET['page']) : 1;
$items_na_stranitse = 10;
$total_payments = getKolichestvoPlatezhi($mysqli, $type);
$obshchee_stranits = ceil($total_payments / $items_na_stranitse);
...
$payments = getVcePlatezhi($mysqli, $items_na_stranitse, $offset, $type);
        ...
        <div class="card">
            <div class="card-header">
                <div class="row w-full">
                    <div class="col">
                        <h3 class="card-title mb-0">Платежи (<?= htmlspecialchars($type_label) ?>)</h3>
                        <p class="text-secondary m-0">Всего платежей: <?= $total_payments ?> штук.</p>
                    </div>
                ...
                <table class="table table-vcenter card-table">
                    <thead>
                        <tr>
                            <th>№ платежа</th>
                            <th>Дата</th>
                            <th><?= ($type === 'iskhodyashchij') ? 'Получатель' : 'Плательщик' ?></th>
                            <th><?= ($type === 'iskhodyashchij') ? 'Плательщик' : 'Получатель' ?></th>
                            <th>Сумма</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (!empty($payments)): ?>
                            <?php foreach ($payments as $platezh): ?>
                                <tr>
                                    <td>
                                        <a href="prosmotr.php?id=<?= htmlspecialchars($platezh['id']) ?>" class="text-primary">
                                            <?= htmlspecialchars($platezh['nomer']) ?>
                                        </a>
                                    </td>
                                    <td class="text-secondary">
                                        <?= htmlspecialchars(date('d.m.Y H:i', strtotime($platezh['data_dokumenta']))) ?>
                                    </td>
                                    <td class="text-secondary">
                                        <?php if ($type === 'iskhodyashchij'): ?>
                                            <?= htmlspecialchars($platezh['naimenovanie_platelshchika'] ?? 'N/A') ?>
                                        <?php else: ?>
                                            <?= htmlspecialchars($platezh['naimenovanie_platelshchika'] ?? 'N/A') ?>
                                        <?php endif; ?>
                                    </td>
                                    <td class="text-secondary">
                                        <?php if ($type === 'iskhodyashchij'): ?>
                                            <?= htmlspecialchars($platezh['naimenovanie_poluchatelya'] ?? 'N/A') ?>
                                        <?php else: ?>
                                            <?= htmlspecialchars($platezh['naimenovanie_poluchatelya'] ?? 'N/A') ?>
                                        <?php endif; ?>
                                    </td>
                                    <td class="text-secondary">
                                        <?= htmlspecialchars(number_format($platezh['summa'], 2, '.', ' ')) ?> 
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="text-center text-secondary p-4">
                                    Платежи не найдены. <a href="redaktirovanie.php">Добавьте первый платеж</a>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
