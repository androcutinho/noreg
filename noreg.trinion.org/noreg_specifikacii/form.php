...
if ($zakaz_id && !$is_edit) {
    $order_data = loadOrderDataForSpecification($mysqli, $zakaz_id, $ot_postavshchika);
    $nomer_order = $order_data['nomer_order'];
    $data_dogovora = $order_data['data_dogovora'];
    $nomer_dogovora = $order_data['nomer_dogovora'];
    $organization_id = $order_data['organization_id'];
    $organization_name = $order_data['organization_name'];
    $kontragenti_id = $order_data['kontragenti_id'];
    $kontragenti_name = $order_data['kontragenti_name'];
    $sotrudniki_id = $order_data['sotrudniki_id'];
    $sotrudniki_name = $order_data['sotrudniki_name'];
...
if ($is_edit) {
    ...
    $organization_id = $spec['id_kontragenti_postavshik'];
    ...
    $org_query = $mysqli->query("SELECT naimenovanie FROM kontragenti WHERE id = " . intval($organization_id));
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$error) {
        if ($is_edit) {
            ...
            $result = createSpecification($mysqli, $_POST, $nomer_order, $zakaz_id, $ot_postavshchika, $pokupatelya);
            ...
            if (!$result['success']) {
                ...
                if (!$line_result['success']) {
                    ...
                    $redirect_url = $ot_postavshchika ? 'index.php?type=postavschik' : 'index.php?type=pokupatel';
