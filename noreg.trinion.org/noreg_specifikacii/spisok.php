...
$type_label = ($type === 'postavschik') ? 'поставщиков' : 'покупателей';
$page_title = 'Спецификации ' . $type_label;
...
$total_specs = getSpecificationsCount($mysqli, $type);
...
$specifications = getAllSpecifications($mysqli, $items_na_stranitse, $offset, $type);
          ...
          <div class="card">
            <div class="card-header">
              <div class="row w-full">
                <div class="col">
                  <h3 class="card-title mb-0">Спецификации <?= htmlspecialchars($type_label) ?></h3>
                ...
                <tbody>
                  <?php if (!empty($specifications)): ?>
                    <?php foreach ($specifications as $spec): ?>
                      <tr>
                        <td><a href="prosmotr.php?id=<?= htmlspecialchars($spec['id']) ?>" class="text-primary"><?= htmlspecialchars($spec['nomer_specifikacii']) ?></a></td>
                        <td class="text-secondary"><?= htmlspecialchars($spec['data_dogovora']) ?></td>
                        <td class="text-secondary"><?= htmlspecialchars($spec['kon_sokrashchyonnoe_naimenovanie'] ?? 'N/A') ?></td>
                        <td class="text-secondary"><?= htmlspecialchars($spec['org_sokrashchyonnoe_naimenovanie'] ?? 'N/A') ?></td>
                  ...
                  <?php else: ?>
                    <tr>
                      <td colspan="5" class="text-center text-secondary p-4">
                        Спецификации еще не добавлены. <a href="redaktirovanie.php">Добавьте первую спецификацию</a>
                      </td>
                    </tr>
                  <?php endif; ?>
                </tbody>
